...
<script>(function(){
  ...
  var _open=false;
  var _pending=false;
  // Wenn Zammad-Widget erscheint (nach WS-Verbindung): pending-Klick anwenden
  var _obs=new MutationObserver(function(){
    var chat=document.querySelector('.zammad-chat');
    if(!chat)return;
    _obs.disconnect();
    if(_pending){chat.classList.add('ldm-visible');_open=true;_pending=false;}
  });
  ...
  function addBtn(){
    ...
    btn.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2h-5l-5 5v-5z"/></svg>';
    document.body.appendChild(btn);
    btn.addEventListener('click',function(){
      var chat=document.querySelector('.zammad-chat');
      if(!chat){_pending=!_pending;return;}
      _open=!_open;
      if(_open)chat.classList.add('ldm-visible');
