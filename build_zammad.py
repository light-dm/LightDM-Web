...
ZAMMAD_BODY = """<script>(function(){
  ...
  function openChat(c){
    ...
    if(h&&!c.classList.contains('zammad-chat-is-open'))setTimeout(function(){h.click();},50);
  }
  function addBtn(){
    ...
    b.addEventListener('click',function(){
      ...
      if(!c){_pending=true;return;}
      if(c.classList.contains('ldm-visible'))c.classList.remove('ldm-visible');
      else openChat(c);
    });
  }
  ...
  function loadZammad(){
    ...
    s.onload=function(){
      ...
      // Auf Chat-Element warten und dann pending anwenden
      var poll=setInterval(function(){
        var c=document.querySelector('.zammad-chat');
        if(!c)return;
        clearInterval(poll);
        if(_pending){openChat(c);_pending=false;}
      },200);
