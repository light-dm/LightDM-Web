...
Button + MutationObserver in separatem window.load-Script für React-Navigation.
"""
import os, re, glob
...
ZAMMAD_BODY = r"""<button id="ldm-zammad-btn" class="open-zammad-chat" aria-label="Chat öffnen"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"/></svg></button>
<script src="https://ticket.light-dm.de/assets/chat/chat-no-jquery.min.js"></script>
<script>(function(){new ZammadChat({chatId:1,show:false,buttonClass:'open-zammad-chat',inactiveClass:'is-inactive',cssAutoload:false,title:'<b>LightDM</b> Support',fontSize:'14px',flat:true});})();</script>
<script>(function(){
...
new MutationObserver(function(){
  if(!document.body.contains(btn)){
    ...
    // ZammadChat neu init wenn Button neu angehängt
    if(window.ZammadChat){
      try{new ZammadChat({chatId:1,show:false,buttonClass:'open-zammad-chat',inactiveClass:'is-inactive',cssAutoload:false,title:'<b>LightDM</b> Support',fontSize:'14px',flat:true});}catch(e){}
    }
  }
